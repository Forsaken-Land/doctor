...
open class HandshakeListener : EventListener {
    ...
    private fun getServiceResponse(packet: ResponsePacket) {
        ...
        serviceResponse = json.decodeFromString(packet.json)
    }
    ...
    private fun handshake(ctx: ChannelHandlerContext) {
        ...
        when (ctx.protocolState()) {
            ProtocolState.HANDSHAKE -> {
                // 发送握手包
                connection.sendPacket(HandshakePacket(0, connection.host, connection.port, ProtocolState.STATUS)).await()
                // 更改连接状态为: 状态
                ctx.setProtocolState(ProtocolState.STATUS)
                val version = serviceResponse.version.protocol
                connection.sendPacket(HandshakePacket(version, connection.host, connection.port, ProtocolState.LOGIN)).await()
            }
            else -> return
        }
